              ...
              {selectedGroup ? (
                <>
                  ...
                  <div className="mb-6">
                    ...
                    {/* Live results notice for same-day tournaments happening today */}
                    {groupStartDate && groupEndDate && groupStartDate === groupEndDate && (() => {
                      const today = new Date();
                      const todayStr = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}`;
                      if (todayStr !== groupStartDate) return null;
                      return (
                        <div className="mb-4 flex items-start gap-2 rounded-md border border-blue-200 dark:border-blue-800/50 bg-blue-50 dark:bg-blue-900/20 px-3 py-2 text-sm text-blue-800 dark:text-blue-300">
                          <svg className="mt-0.5 h-4 w-4 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20">
                            <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
                          </svg>
                          <span>
                            {t.pages.tournamentResults.tournamentStatus.liveResultsNotice}
                            {' '}
                            <a
