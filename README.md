...
# Download the org data (districts/clubs from schack.se + refresh geocode caches).
# These files are not tracked in git; `pnpm dev` will refuse to start without them.
pnpm data:clubs
...
> **Note on `public/data/`:** the downloadable org data (`clubs-by-district.json`,
> `districts.json`, `organizations-all.json`) is git-ignored and regenerated by
> `pnpm data:clubs` (it's also rebuilt on every deploy). The geocode caches
> (`geocodes.json`, `club-geocodes.json`) *are* committed — they're expensive to
> rebuild (Nominatim) and act as the incremental cache.
...
- `pnpm data:clubs` - Download org data + refresh geocode caches
